...
def analyze(df):
    ...
    if "Bullish" in trend:
        ...
        t1 = price + atr * 2
        t2 = price + atr * 4
        option = f"✅ CALL Option (Strike ~ {round(price)}, Exp 2–3 weeks)"
    elif "Bearish" in trend:
        entry = ema20
        stop = price + atr * 1.5
        t1 = price - atr * 2
        t2 = price - atr * 4
        option = f"✅ PUT Option (Strike ~ {round(price)}, Exp 2–3 weeks)"
    else:
        entry = price
        stop = price - atr * 1.2
        t1 = price + atr * 1.5
        t2 = price + atr * 2
        option = "⚠️ No strong setup — wait for breakout"
    ...
    if df is None:
        ...
        with col2:
            ...
            st.write(
                ...
                f"Confidence score is **{r['score']}%**, indicating trade quality."
            )
