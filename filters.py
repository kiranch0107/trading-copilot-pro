...
def debug_adx(adx_value: Any, cfg: Config, debug: List[Dict]) -> bool:
    ...
    ok = val >= cfg.min_adx
    debug.append(structured_debug("adx", value=val, min=cfg.min_adx, passed=ok))
    return ok
...
def debug_weekly_slope(weekly_slope: Any, min_weekly_slope: float, debug: List[Dict]) -> bool:
    ...
    debug.append(structured_debug("weekly_slope", value=val, min=min_weekly_slope, passed=ok))
    return ok
...
def debug_earnings(days_to_earnings: Any, cfg: Config, debug: List[Dict]) -> bool:
    ...
    debug.append(structured_debug("earnings", days=days, blocked=not ok, passed=ok))
    return ok
...
def candidate_passes_checks(candidate: Dict[str, Any], cfg: Config, debug: Optional[List[Dict]] = None) -> bool:
    ...
    if dte < max(1, cfg.min_dte) and candidate.get("ticker", "").upper() not in cfg.force_allow_same_day:
        debug.append(structured_debug("dte", value=dte, min=cfg.min_dte, passed=False))
        return False
    debug.append(structured_debug("dte", value=dte, min=cfg.min_dte, passed=True))

    if dte > cfg.max_dte:
        debug.append(structured_debug("dte_max", value=dte, max=cfg.max_dte, passed=False))
        return False
    ...
    debug.append(structured_debug("liquidity", volume=vol, min_volume=cfg.min_option_volume, oi=oi, min_oi=cfg.min_option_oi, passed=(vol_ok and oi_ok)))
    ...
    if mid <= 0 or math.isnan(mid):
        debug.append(structured_debug("price", mid=mid, passed=False, reason="mid<=0 or NaN"))
        return False
    if mid < 0.05 and (vol < 1000 or oi < 200):
        debug.append(structured_debug("price", mid=mid, passed=False, reason="tiny mid with low liquidity"))
        return False
    debug.append(structured_debug("price", mid=mid, spread=spread, passed=True))
    return True
